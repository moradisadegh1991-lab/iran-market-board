...
**⚠️ اگر با اخطار «۱۰۰٪ دقیقه Actions مصرف شد» از گیت‌هاب مواجه شدید:** نسخه قبلی همین فایل ۵۵ دقیقه در هر ساعت runner را بیدار نگه می‌داشت — حدود ۴۰٬۰۰۰ دقیقه در ماه. روی ریپوی **عمومی** رایگان بود، ولی روی ریپوی **خصوصی** کل سهمیه رایگان ۲٬۰۰۰ دقیقه‌ای ماهانه را در کمتر از دو روز تمام می‌کرد — و از آن لحظه به بعد **هیچ** workflow دیگری در همان ریپو (حتی بیلد اندروید) هم اجرا نمی‌شود تا سهمیه ماه بعد ریست شود. اگر به این خطا خورده‌اید، تا شروع دوره صورتحساب بعدی کاری از گیت‌هاب اکشن‌ها ساخته نیست؛ یا صبر کنید، یا ریپو را عمومی کنید (دقایق روی ریپوی عمومی نامحدود و همیشه رایگان است).

‏فایل جدید (`paper-tick.yml`) دیگر داخل هر اجرا حلقه نمی‌زند — هر اجرا فقط یک‌بار تیک می‌زند و کمتر از ۱ دقیقه طول می‌کشد. ساعتی که اجرا شود یعنی حدود ۷۲۰ دقیقه در ماه، خیلی کمتر از سهمیه رایگان، حتی کنار workflow بیلد اندروید.

**‏روش پیشنهادی برای تیک مکرر (هر چند دقیقه)، بدون مصرف حتی یک دقیقه از گیت‌هاب:** از یک سرویس cron رایگان بیرون از گیت‌هاب استفاده کنید — چون بیرون گیت‌هاب است، هیچ ربطی به این سهمیه ندارد:

۱. ‏در [cron-job.org](https://cron-job.org) یک job جدید بسازید، هر ۵ دقیقه، روی این دو آدرس:
```
https://YOUR-APP.vercel.app/api/paper/tick?secret=ADMIN_SECRET
https://YOUR-APP.vercel.app/api/swing-live/tick?secret=ADMIN_SECRET
```
۲. ‏همین فایل گیت‌هاب را به‌عنوان پشتیبان ساعتی نگه دارید (اگر cron-job.org قطعی داشت) — کاری لازم نیست، همین حالا با تنظیمات پیش‌فرض همین کار را می‌کند.

‏اگر ترجیح می‌دهید همه‌چیز داخل گیت‌هاب بماند و هزینه‌اش را می‌پذیرید (فقط روی ریپوی **عمومی**)، از تب Actions، «Run workflow» را بزنید و گزینه `enable_loop` را روشن کنید — حلقه ۵۵ دقیقه‌ای قدیمی هنوز در فایل هست، ولی دیگر پیش‌فرض نیست.

‏این دو Secret را در گیت‌هاب بسازید (`Settings → Secrets and variables → Actions`):
...
| پروفایل | موتور آموخته | تنظیمات ثابت | تفاوت |
|---|---|---|---|
| محتاط | ۶۴٫۸٪ | ۶۴٫۸٪ | ‎+۰٫۰ |
| متعادل | ۵۹٫۴٪ | ۵۳٫۶٪ | ‎+۵٫۸ |
| جسور | ۴۲٫۸٪ | ۳۷٫۹٪ | ‎+۴٫۹ |

**نوسان‌گیر:** هر اجرا (تک‌ارز، سبد، غربال خودکار) و هر **جلسه برخط پایان‌یافته** در حافظه ثبت می‌شود و به «سابقه هر ارز» تبدیل می‌شود. غربال خودکار این سابقه را در رتبه‌بندی لحاظ می‌کند، با سه قید:

- ‏معیار سابقه **مزیت نسبت به خرید و نگه‌داری** است، نه بازده خام. ارزی که ۴۰٪ داده وقتی صرفاً نگه‌داشتنش ۶۰٪ می‌داده، وسیله بدی برای نوسان‌گیری بوده و نباید بالا بیاید.
- ‏سابقه با تعداد اجرا **کوچک می‌شود**: یک اجرا تقریباً بی‌اثر است، بیست اجرا اثر معنادار دارد.
- ‏سقف اثر سابقه محدود است، پس فقط می‌تواند رتبه‌های نزدیک را جابه‌جا کند و هرگز نتیجه داده فعلی را وارونه نمی‌کند. `npm run test:swinghist` این سه قید را تضمین می‌کند، از جمله با ساختن یک سابقه عمداً گمراه‌کننده و بررسی اینکه انتخاب را برنگرداند.
...
مراحل ساخت در `android-app/README.md`.
...
- سهمیه رایگان گیت‌هاب اکشن‌ها روی ریپوی **خصوصی** ماهی ۲٬۰۰۰ دقیقه است (روی ریپوی عمومی نامحدود). حالت پیش‌فرض همین فایل (تیک تکی، ساعتی) حدود ۷۲۰ دقیقه در ماه مصرف می‌کند؛ حالت حلقه‌ای اختیاری (`enable_loop`) حدود ۴۰٬۰۰۰ دقیقه می‌خواهد و فقط برای ریپوی عمومی مناسب است.
